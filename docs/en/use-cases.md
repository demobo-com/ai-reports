...
- **The hardware can be trivially dumb — a media player (image / video / animation) pulling pre-rendered AI assets from cloud storage.** No browser, no live rendering, no HTML runtime on the dongle. All generation happens upstream in the cloud; the dongle just plays what's queued.
...
- **Motion content, not just static HTML.** Because generation is decoupled from display, the dongle can play anything the cloud can render — short video loops, animated transitions, Ken Burns-style motion on photos — on the same cheap hardware.
- **Real-time on-premise correction.** Staff standing in front of the TV can say *"bigger headline"*, *"swap to today's soup"*, or *"add a QR code to the lunch menu"*. The cloud regenerates the asset, pushes it to the dongle, and the display updates within seconds. No back-and-forth with an external designer, no file re-uploads, no CMS portal.
...
- **Cheap to scale.** Generation happens once per variation and caches on a CDN. A chain with 50 locations pays for one render, not 50. Fleet-wide refreshes are just cache invalidations.
...
3. **Companion web app or mobile app** — structured form with templates, media upload, scheduling. Most control, heaviest UX — the user has to open the app.

**Recommended MVP: option 2 (email / WhatsApp).** Cheapest hardware, fastest to ship, and fits how small-business staff already communicate every day. Voice can ship as a v2 upgrade once there's a fleet to justify the mic BOM.
