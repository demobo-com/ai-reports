...
本文整理主要 agent 行動類別的當前技術成熟度，包括代表性 benchmark、目前能做到的事，以及仍然做不好的地方。章節順序依實務落地成熟度排列（最成熟的在前），與 [Benchmark 儀表板]({{ '/zh-tw/benchmark/' | relative_url }}) 保持一致。每個章節標題附上 0-100 的成熟度分數（綠 ≥ 70、橘 40-69、紅 < 40）。

## 更新紀錄

<!-- 新資料在上。每週排程 agent 會在此處追加；超過約 20 筆時將舊資料移至封存頁。 -->

- **2026-04-20** — 發佈初始快照。收錄資料：7 大類 15 個 benchmark、約 85 筆資料，時間跨度 2023-07 至 2026-04。
...
## 1. CLI / 程式開發 {% include mat-badge.html score=85 %}
{:#cli_coding}
...
這些 benchmark 主要評估 coding agent 在真實軟體工程任務上的表現，例如修 bug、補測試、處理 GitHub issue，以及 Linux 終端機任務。

截至 2026 年，這是最成熟的一類 agent。
官方 SWE-bench 排行顯示頂尖系統已達到 **高 80% 區間**，Claude Opus 4.7 在 SWE-bench Verified 上達到 **87.6%**；Terminal-Bench 2 的最佳成績約為 80%，由 GPT-5.4 / ForgeCode 組合締造。
...
## 2. API / 工具使用 {% include mat-badge.html score=72 %}
{:#tool_use}
...
## 3. 試算表 / 檔案 {% include mat-badge.html score=58 %}
{:#spreadsheet}
...
## 4. 瀏覽器 {% include mat-badge.html score=55 %}
{:#browser}
...
WebVoyager 之後被 Surfer 2 推到 97% 以上。BrowseComp 則是更難的 benchmark，專門測試在真實網路中持續搜尋與找到難查答案的能力；其人類基準僅 **29.2%**，可見題目本身對人類也相當困難。
...
## 5. 溝通 / 助理 {% include mat-badge.html score=38 %}
{:#assistant}
...
## 6. GUI / 電腦操作 {% include mat-badge.html score=42 %}
{:#gui_computer}
...
至 2026 年 Q1，最強模型（Claude Opus 4.7）在 OSWorld-Verified 上達到 **78.0%**，已超越原論文的人類基準；但從實務角度看，GUI / computer use 仍明顯不如 CLI 或結構化 tool use 穩定。
...
## 7. 裝置 / 機器人 {% include mat-badge.html score=22 %}
{:#robotics}
